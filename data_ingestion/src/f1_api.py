...
class F1API() : 
    ...
    def build_param_url(self, url_param : dict) -> None : 
        ...
        if (not url) is False : 
            ...
            if (not offsets) is False : 
                url_param["url"] = [f'{url}/?limit=100&offset={offset}' for offset in offsets]
                return url_param
    ...
    def build_url_race_data(self) : 
        ...
        results_url = [self.build_param_url(url) for url in tqdm(base_url, desc="Processing URLs")]

        clean_results = [urls for urls in results_url if (not urls) is False]

        season_and_category = set([(race["season"], race["category"]) for race in clean_results])
        url_by_seasons_and_category = [{
            'season' : s_c[0],
            'category' : s_c[1],
            'url' : [race["url"] for race in clean_results if race['season'] == s_c[0] and race["category"] == s_c[1]]
    ...
    def fetch_races_data(self) : 
        ...
        data = [self.build_race_data(param) for param in tqdm(params, desc = "fetching races data")]
    ...
    def fetch_seasons_results(self) : 
        ...
        data = [self.build_races_results(param) for param in tqdm(params, desc="fetching results")]
