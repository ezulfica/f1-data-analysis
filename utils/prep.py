...
def align_nested_structs(dataframes):
    ...
    dataframes = [
        df.with_columns(
            [pl.lit(None).alias(col) for col in all_columns if col not in df.columns]
        )
        for df in dataframes
    ]
    ...
    for df in dataframes:
        for col, dtype in zip(df.columns, df.dtypes):
            if isinstance(dtype, pl.Struct):
                ...
                list_struct_columns[col] = list_struct_columns.get(col, set()) | set(
                    dtype.inner.fields
                )
            ...
            if col in df.columns:
                ...
                if missing_fields:
                    dataframes[i] = df.with_columns(
                        ...
                        .struct.update(
                            {
                                field: pl.lit(None).cast(pl.Utf8)
            ...
            if col in df.columns:
                ...
                if missing_fields:
                    dataframes[i] = df.with_columns(
                        df[col].list.eval(
                            pl.struct(
                                {f: pl.lit(None).cast(pl.Utf8) for f in missing_fields}
                            )
                        )
                    )
