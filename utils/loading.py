...
def file_to_load(folder: str) -> dict:
    ...
    files_list = {
        foldername: [str(filename) for filename in parquet_files if filename.parent.name == foldername] 
...
def load_into_bigquery(data: pd.DataFrame, project: str, dataset: str, schema_json: dict, bq_client: bigquery.Client) -> None:
    for table, df in data.items():
        table_id = f"{project}.{dataset}.{table}"
        schema = [bigquery.SchemaField(name, data_type) for name, data_type in schema_json[table].items()]
